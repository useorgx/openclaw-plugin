...
async function fetchKickoffContextSafe(
  client: OrgXClient,
  payload: KickoffContextRequest
): Promise<KickoffContext | null> {
  try {
    const anyClient = client as unknown as { getKickoffContext?: (p: KickoffContextRequest) => Promise<KickoffContextResponse>; rawRequest?: (...args: any[]) => Promise<unknown> };
...
function renderKickoffMessage(input: {
  baseMessage: string;
  ...
  const title = (kickoff.task?.title ?? kickoff.workstream?.title ?? kickoff.initiative?.title ?? "").trim();
  ...
  if (risks.length > 0) {
    ...
    lines.push("");
      ...
      const title = typeof (item as any)?.title === "string" ? String((item as any).title).trim() : "";
      const id = typeof (item as any)?.id === "string" ? String((item as any).id).trim() : "";
      const label = title || id || "decision";
      lines.push(`- Decision: ${label}`);
    }
    for (const item of artifacts) {
      const title = typeof (item as any)?.title === "string" ? String((item as any).title).trim() : "";
      const id = typeof (item as any)?.id === "string" ? String((item as any).id).trim() : "";
      const label = title || id || "artifact";
  ...
  if (input.requiredSkills.length > 0) lines.push(`- Skills: ${input.requiredSkills.join(", ")}`);
...
export function createHttpHandler(
  ...
  function readFileTailSafe(pathname: string, maxChars = 64_000): string {
    try {
  ...
  function fileUpdatedAtEpochMs(pathname: string, fallbackEpochMs: number): number {
    try {
  ...
  function spawnCodexSliceWorker(input: {
    ...
    if (workerKind === "mock") {
      const scriptPath = resolve(join(__filename, "..", "..", "scripts", "mock-autopilot-slice-worker.mjs"));
      ...
      if (!slice) {
        ...
        run.updatedAt = now;
	        ...
	        if (pid && pidAlive(pid)) {
	          ...
	          const outputParsed = outputTail ? parseSliceResult(outputTail) : null;
    ...
    if (url.startsWith("/orgx/api/")) {
      ...
      if (method === "POST" && isAgentLaunchRoute) {
        try {
          ...
          const kickoff = await fetchKickoffContextSafe(client, {
            initiative_id: initiativeId,
          ...
          const kickoffRendered = renderKickoffMessage({
            baseMessage,
            kickoff,
            domain: executionPolicy.domain,
            requiredSkills: executionPolicy.requiredSkills,
          ...
          if (taskId) {
            ...
            await syncParentRollupsForTask({
              ...
              orgx_plugin_version: (config.pluginVersion ?? "").trim() || null,
          ...
          const includeVerificationRaw =
            ...
            null;
	          ...
	          const fastAckRaw =
	            ...
	            searchParams.get("fastAck") ??
	            searchParams.get("fast_ack") ??
	            null;
	          const fastAck =
	          ...
	          const maybeDispatchFallback = async () => {
	            if (
	              ...
	              return await dispatchFallbackWorkstreamTurn({
	                initiativeId,
	          ...
	          if (!fastAck) {
	            // Preserve legacy semantics for API callers/tests: play blocks until one tick completes.
	            await tickAutoContinueRun(run);
	            fallbackDispatch = await maybeDispatchFallback();
	          } else {
	            // UI callers can opt into a faster ack: start one dispatch tick, but avoid holding
	            // the HTTP response open indefinitely. If the tick is slow, ack quickly and finish
	            // dispatch in the background.
	            const tickPromise = tickAutoContinueRun(run);
	            ...
	            if (!tickCompleted) {
	              void tickPromise
	                ...
	                .catch(() => {
	                  // best effort; errors will surface via activity feed/polling
	                });
      ...
      if (method === "POST" && isMissionControlAutoContinueTickRoute) {
        try {
          const payload = await parseJsonRequest(req);
          const initiativeId =
            (pickString(payload, ["initiativeId", "initiative_id"]) ??
          ...
          if (initiativeId) {
            ...
            await tickAutoContinueRun(run);
            sendJson(res, 200, { ok: true, initiativeId, run });
            return true;
          }

          await tickAllAutoContinue();
