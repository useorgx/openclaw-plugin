  ...
  return (
    <div className="relative">
      <button
        ...
        className="rounded-md border border-strong bg-white/[0.04] px-2 py-1 text-micro text-primary transition-colors hover:bg-white/[0.08] disabled:opacity-40"
      >
      ...
      {open && (
        <div className="surface-tier-2 absolute right-0 z-20 mt-1.5 w-[320px] rounded-xl p-2.5 shadow-[0_20px_40px_rgba(0,0,0,0.45)]">
          <label className="text-micro uppercase tracking-[0.08em] text-secondary">
            Link dependencies
          </label>
          <input
            value={query}
          ...
          <div className="mt-2 flex max-h-[84px] flex-wrap gap-1.5 overflow-y-auto rounded-md border border-white/[0.08] bg-black/20 p-1.5">
            {selected.length === 0 ? (
              <span className="text-micro text-muted">No links selected</span>
            ) : (
              selected.map((id) => {
                ...
                return (
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-full border border-strong bg-white/[0.05] px-2 py-0.5 text-micro text-primary transition-colors hover:bg-white/[0.1]"
                    title={title}
                  >
          ...
          <div className="mt-2 max-h-[150px] space-y-1 overflow-y-auto rounded-md border border-white/[0.08] bg-black/20 p-1.5">
            {filteredNodes.length === 0 ? (
              <div className="px-1 text-micro text-muted">No matching nodes</div>
            ) : (
                ...
                return (
                  ...
                  >
                    <span className="min-w-0 pr-2">
                      ...
                      <span className="block truncate text-muted">{node.id.slice(0, 8)}…</span>
                    </span>
          ...
          <div className="mt-2 flex items-center justify-end gap-1.5">
            <button
              ...
              className="rounded-md border border-strong px-2 py-1 text-micro text-secondary transition-colors hover:bg-white/[0.08]"
            >
              Cancel
            </button>
            <button
