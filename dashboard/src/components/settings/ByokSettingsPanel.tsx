...
export function ByokSettingsPanel({
  ...
  return (
    <div className="flex min-h-0 flex-col gap-6">
      <div>
        <h3 className="text-[22px] font-semibold leading-tight text-white">Provider keys</h3>
        <p className="mt-2 max-w-2xl text-body leading-relaxed text-secondary">
          Bring your own provider credentials, keep the source policy obvious, and make launch readiness legible
          before anyone starts a run.
        </p>
        {configuredCount === 0 ? (
          <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-amber-200/20 bg-amber-200/10 px-3 py-1 text-caption text-amber-100/85">
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
          <div className="rounded-xl border border-white/[0.08] bg-white/[0.02] p-4">
            ...
            <p className="mt-1.5 text-body leading-relaxed text-secondary">
              If you set an env var (for example{' '}
              <code className="rounded bg-black/40 px-1">OPENAI_API_KEY</code>), it remains the fallback until a
              saved key overrides it for that provider.
            </p>
          </div>
          <div className="grid gap-2 sm:grid-cols-3 xl:grid-cols-1">
            <div className="rounded-xl border border-white/[0.08] bg-black/20 px-3.5 py-3">
              <p className="text-micro uppercase tracking-[0.14em] text-muted">Configured keys</p>
              <p className="mt-2 text-[22px] font-semibold leading-none text-white">{configuredCount} / 3</p>
            </div>
            <div className="rounded-xl border border-white/[0.08] bg-black/20 px-3.5 py-3">
              <p className="text-micro uppercase tracking-[0.14em] text-muted">Ready providers</p>
              <p className="mt-2 text-[22px] font-semibold leading-none text-white">{readinessMetrics.readyProviders}</p>
            </div>
            <div className="rounded-xl border border-white/[0.08] bg-black/20 px-3.5 py-3">
              <p className="text-micro uppercase tracking-[0.14em] text-muted">Pending saves</p>
              <p className="mt-2 text-[22px] font-semibold leading-none text-white">{readinessMetrics.pendingSaves}</p>
            </div>
          </div>
        </div>
      </ByokSubspace>
      ...
      <ByokSubspace
        step="Providers"
        title="Providers"
        description="Manage each provider independently: save, clear, then probe readiness."
      >
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="text-[18px] font-semibold text-white">{provider.label}</p>
                    <span
                      className={cn(
                        'rounded-full border px-2 py-0.5 text-micro uppercase tracking-[0.12em]',
                    ...
                    {source !== 'none' ? <span className="chip">source: {source}</span> : null}
                    {providerHealth ? (
                      <span
                        className={cn(
                          'chip',
                  ...
                  <p className="mt-2 text-caption text-muted">
                    Fallback env: <code className="rounded bg-black/40 px-1">{provider.envVar}</code>
                    {masked ? <span className="ml-2 text-secondary">Stored key {masked}</span> : null}
                  </p>
                </div>
              ...
              >
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-white/[0.06] pt-3">
                  <span className="text-caption text-muted">
                    {dirty[provider.id] ? 'Unsaved changes' : hasStoredKey ? 'Stored locally' : 'No local key saved'}
                  </span>
                  <div className="flex flex-wrap items-center gap-2">
                  <button
                    type="submit"
