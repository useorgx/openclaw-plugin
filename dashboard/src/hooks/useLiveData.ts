...
// ── Group Work Slice Projections by Initiative → Workstream ──────

export interface WorkstreamGroup {
  workstreamId: string;
  workstreamTitle: string;
  slices: WorkSliceProjectionV2[];
...
export interface GroupedSessions {
  ...
  ungrouped: WorkSliceProjectionV2[];
  lastActivityAt: string | null;
}
...
function latestTimestamp(a: string | null, b: string | null): string | null {
  if (!a) return b;
  if (!b) return a;
  return toEpoch(a) >= toEpoch(b) ? a : b;
}

/**
 * Groups work slice projections by initiativeId → workstreamId.
 * Each group tracks the most recent `updatedAt` as `lastActivityAt`.
 */
export function groupSessionsByInitiative(
  projections: WorkSliceProjectionV2[] | undefined | null
): GroupedSessions {
  const empty: GroupedSessions = { groups: [], ungrouped: [], lastActivityAt: null };
  if (!projections || projections.length === 0) return empty;

  // initiative → workstream → slices
  const initiativeMap = new Map<
    string,
    {
      title: string;
      workstreams: Map<string, { title: string; slices: WorkSliceProjectionV2[] }>;
    }
  >();
  const ungrouped: WorkSliceProjectionV2[] = [];

  for (const slice of projections) {
    const iId = slice.lineage?.initiativeIds?.[0] ?? null;
    const wId = slice.lineage?.workstreamIds?.[0] ?? null;

    if (!iId) {
      ungrouped.push(slice);
      continue;
    }

    let initEntry = initiativeMap.get(iId);
    if (!initEntry) {
      const iTitle = slice.lineage?.initiativeTitles?.[0] ?? iId;
      initEntry = { title: iTitle, workstreams: new Map() };
      initiativeMap.set(iId, initEntry);
    }

    const wsKey = wId ?? '__no_workstream__';
    let wsEntry = initEntry.workstreams.get(wsKey);
    if (!wsEntry) {
      const wTitle = slice.lineage?.workstreamTitles?.[0] ?? wId ?? 'Unassigned';
      wsEntry = { title: wTitle, slices: [] };
      initEntry.workstreams.set(wsKey, wsEntry);
    }
    wsEntry.slices.push(slice);
  }
  ...
  for (const [initiativeId, entry] of initiativeMap) {
    let initLastActivity: string | null = null;
    const workstreams: WorkstreamGroup[] = [];

    for (const [workstreamId, wsEntry] of entry.workstreams) {
      let wsLastActivity: string | null = null;
      for (const slice of wsEntry.slices) {
        wsLastActivity = latestTimestamp(wsLastActivity, slice.updatedAt);
      }

      workstreams.push({
        workstreamId,
        workstreamTitle: wsEntry.title,
        slices: wsEntry.slices,
        lastActivityAt: wsLastActivity,
      });
      initLastActivity = latestTimestamp(initLastActivity, wsLastActivity);
    }

    // Sort workstreams by most recently active first
    workstreams.sort(
      (a, b) => toEpoch(b.lastActivityAt) - toEpoch(a.lastActivityAt)
    );
    ...
    groups.push({
      ...
      workstreams,
      lastActivityAt: initLastActivity,
    });
    globalLastActivity = latestTimestamp(globalLastActivity, initLastActivity);
  }

  // Also consider ungrouped for global last activity
  for (const slice of ungrouped) {
    globalLastActivity = latestTimestamp(globalLastActivity, slice.updatedAt);
  }

  // Sort initiative groups by most recently active first
  groups.sort(
    (a, b) => toEpoch(b.lastActivityAt) - toEpoch(a.lastActivityAt)
  );

  return { groups, ungrouped, lastActivityAt: globalLastActivity };
}
